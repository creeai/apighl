...
export class Model {
  ...
  async updateN8NWebhookUrl(resourceId: string, n8n_webhook_url: string): Promise<void> {
    try {
      console.log(`💾 Atualizando webhook N8N para ${resourceId}: ${n8n_webhook_url}`);
      
      const result = await pool.query(
        `UPDATE installations 
         SET n8n_webhook_url = $1, updated_at = NOW() 
         WHERE location_id = $2 OR company_id = $2`,
        [n8n_webhook_url, resourceId]
      );
