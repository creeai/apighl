  ...
  if (eventType === 'UNINSTALL') {
      ...
      try {
        ...
        if (result.success) {
          ...
          // ✅ NOVO: Enviar para N8N se webhook configurado
          try {
            const installation = await ghl.model.getInstallationInfo(locationId);
            
            if (installation?.n8n_webhook_url) {
              console.log(`🔄 Enviando dados OUTBOUND para N8N webhook: ${installation.n8n_webhook_url}`);
              
              const n8nPayload = {
                event: 'whatsapp_message_sent',
                message_type: 'outbound',
                direction: 'outbound',
                instanceName: installation.evolutionInstanceName,
                resourceId: locationId,
                phoneNumber: phoneNumber,
                message: message,
                timestamp: new Date().toISOString(),
                ghl_contact_id: contactId,
                ghl_message_id: messageIdToUpdate,
                source: 'ghl_crm',
                status: 'delivered',
                evolution_api_response: result.data
              };
              
              // Enviar para N8N (não bloqueia o fluxo principal)
              axios.post(installation.n8n_webhook_url, n8nPayload, {
                timeout: 5000,
                headers: {
                ...
                console.log(`✅ Dados OUTBOUND enviados com sucesso para N8N`);
              }).catch((error: any) => {
                console.error(`❌ Erro ao enviar OUTBOUND para N8N:`, error.message);
              });
            }
          } catch (error: any) {
            console.error(`❌ Erro ao processar webhook N8N OUTBOUND:`, error.message);
          }
      ...
      const messageData = req.body.data;
        ...
        try {
          ...
          if (result.success) {
            ...
            // ✅ NOVO: Enviar para N8N se webhook configurado
            try {
              const installation = await ghl.model.getInstallationInfo(resourceId);
              
              if (installation?.n8n_webhook_url) {
                console.log(`🔄 Enviando dados INBOUND para N8N webhook: ${installation.n8n_webhook_url}`);
                ...
                const n8nPayload = {
                  ...
                  ghl_conversation_id: result.data?.conversationId,
                  source: 'whatsapp',
                  message_type_detected: messageType,
                ...
                axios.post(installation.n8n_webhook_url, n8nPayload, {
                  ...
                  console.log(`✅ Dados INBOUND enviados com sucesso para N8N`);
                }).catch((error: any) => {
                  console.error(`❌ Erro ao enviar INBOUND para N8N:`, error.message);
                });
              }
            } catch (error: any) {
              console.error(`❌ Erro ao processar webhook N8N INBOUND:`, error.message);
            }
            ...
            return res.status(200).json({
              ...
              data: {
                ...
                phoneNumber: inboundPhoneNumber,
                message: inboundMessageText,
                n8n_webhook_sent: false // Será atualizado se webhook N8N estiver configurado
              }
            });
          } else {
...
// ✅ NOVO: Rota para configurar webhook N8N
app.post("/integration/configure-n8n-webhook", 
  ghlCredentialsValidator.validateGHLCredentials, // Requer credenciais GHL válidas
  async (req: Request, res: Response) => {
  try {
    const { resourceId, n8n_webhook_url } = req.body;
    
    if (!resourceId || !n8n_webhook_url) {
      return res.status(400).json({
        success: false,
    ...
    // Logs de configuração simplificados
    console.log(`🔧 Configurando webhook N8N para: ${resourceId}`);
    ...
    // Atualizar webhook N8N
    await ghl.model.updateN8NWebhookUrl(resourceId, n8n_webhook_url);
    
    // Logs de configuração simplificados
    console.log(`✅ Webhook N8N configurado com sucesso para: ${resourceId}`);
